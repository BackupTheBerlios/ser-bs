...
sub db_get{
	...
	$sql = qq{SELECT COUNT(DISTINCT sip_callid) FROM missed_calls WHERE (sip_method='INVITE')AND unix_timestamp(timestamp)>unix_timestamp(NOW())-$time};
...
my $name="calls_totalmissed"."_".$time."_Z".$zoom;
qx(/usr/bin/perl ../ser_rrd/scripts/ser_graph.pl calls_totalmissed $name $time $zoom);
...
print "</head>\n<body>\n";
print "<center>";
print "<FORM action='/cgi/sercallmissed.cgi' method=get>\n";
print "<h1>Analysis of missed calls</h1>\n";
print "<br>\n";
print "Zoom factor\n";
...
print "</FORM>\n";
print "Missed calls since last $min minutes: $result \n";
